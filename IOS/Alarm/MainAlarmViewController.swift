...
class MainAlarmViewController: UITableViewController{
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if isEditing {
            performSegue(withIdentifier: Id.editSegueIdentifier, sender: SegueInfo(curCellIndex: indexPath.row, isEditMode: true, label: alarmModel.alarms[indexPath.row].label, mediaLabel: alarmModel.alarms[indexPath.row].mediaLabel, mediaID: alarmModel.alarms[indexPath.row].mediaID,  enabled: alarmModel.alarms[indexPath.row].enabled))
        }
    }
    ...
    @IBAction func switchTapped(_ sender: UISwitch) {
        ...
        if sender.isOn {
            print("switch on")
            //alarmScheduler.setNotificationWithDate(alarmModel.alarms[index].date, soundName: alarmModel.alarms[index].mediaLabel, index: index)
            tableView.reloadData()
        }
        else {
            print("switch off")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Id.addSegueIdentifier {
            addEditController.navigationItem.title = "Add Reminder"
            addEditController.segueInfo = SegueInfo(curCellIndex: alarmModel.count, isEditMode: false, label: "", mediaLabel: "bell", mediaID: "", enabled: false)
        }
        else if segue.identifier == Id.editSegueIdentifier {
            addEditController.navigationItem.title = "Edit Reminder"
